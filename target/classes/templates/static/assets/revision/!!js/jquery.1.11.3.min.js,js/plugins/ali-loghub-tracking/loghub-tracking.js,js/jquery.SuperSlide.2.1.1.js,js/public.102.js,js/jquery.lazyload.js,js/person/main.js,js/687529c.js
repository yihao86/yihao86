...
/* COMBO: filename = js/person/main.js, type = application/javascript, resp = 200 */

// 左侧悬浮
function fix() {
    var scrollTop = $(document).scrollTop();
    if (scrollTop > 300) {
        $('.user-wrapper .wrap-l').addClass("fixed");
        $('.user-banner .avatar').addClass("fixed");
    }
    else {
        $('.user-wrapper .wrap-l').removeClass("fixed");
        $('.user-banner .avatar').removeClass("fixed");
    }
}
...
// 查看vip权限
$('.other-vip .list-link .fl').hover(function () {
    $(this).parents('.list').find('.img-r').css('left', '0');
}, function () {
    $(this).parents('.list').find('.img-r').css('left', '-101%');
});

// 一个很应付的显示隐藏
$('.record-win').on('click', '.close', function () {
    $('.pay-win').addClass('hide')

});
$('.wrap-r').on('click', '.jl', function () {
    $('.pay-win').removeClass('hide')
...
$('.base-win').on('click', '.del', function () {
    ...
    $.ajax({
        url: Url.ajaxDeleteAlbumUrl,
        data: {id: $id, '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')},
        dataType: 'JSON',
        type: 'post',
        success: function (response) {
            if (response.error === 0) {
                $(".base-win").addClass('hide');
                $('.list[data-ralation=' + $id + ']').remove();
            } else {
...
$('a[data-evevt-js=cancel-follow-js]').click(function () {
    if (Param.uid == 0) {
        toLogin();
        return false;
    }
...
/* COMBO: filename = js/687.js, type = text/plain, resp = 404 */
