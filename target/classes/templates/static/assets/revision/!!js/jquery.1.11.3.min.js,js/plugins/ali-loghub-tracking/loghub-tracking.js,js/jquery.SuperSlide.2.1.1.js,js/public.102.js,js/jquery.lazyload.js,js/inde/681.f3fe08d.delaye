...
/* COMBO: filename = js/software/software.v2.js, type = application/javascript, resp = 200 */
/**
 * Created by Mr.D on 2017/6/22.
 */
$(function () {
    $('.menu-box .mb-item').each(function () {
        var k = true;
        $(this).find('.more-cut-btn').on('click', function () {
            $(this).siblings('.mm-item-bod').find('.more-item-ctrl').css({
                display: k ? 'block' : 'none'
            });
            $(this).html(k ? '收起详情' : '展开详情');
            k = !k;
        })
    })
