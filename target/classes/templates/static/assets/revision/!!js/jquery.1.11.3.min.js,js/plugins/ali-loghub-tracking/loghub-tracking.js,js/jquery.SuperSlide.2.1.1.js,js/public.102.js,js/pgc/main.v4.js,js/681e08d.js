...
/* COMBO: filename = static/plugins/ali-loghub-tracking/loghub-tracking.static, type = application/javascript, resp = 200 */
...
/* COMBO: filename = static/jquery.SuperSlide.2.1.1.static, type = application/javascript, resp = 200 */
/*!
 * SuperSlide v2.1.1
 * 轻松解决网站大部分特效展示问题
...
/* COMBO: filename = static/public.102.static, type = application/javascript, resp = 200 */
function ConfigFun() {
    var host = 'https://ajax.yihao86';
    var config = {
...
$(function () {
    ...
    $('#empty-btn').on('click', function () {
        $.get('//yihao86/search/del-cookie', {name: 'qingkong'}, function (data) {
    ...
    $('.cookie').on('click', function () {
        var _this = $(this);
        $.get('//yihao86/search/del-cookie', {name: $(this).parents('p').find('a').html()}, function (data) {
    ...
    $('#submit-search').on('click', function () {
        ...
        window.location.href = '//yihao86/search/video?keyWorld=' + $keyWorld + '&identify=' + identify;
    });
    ...
    (function () {
        ...
        (function () {
            ...
            hm.src = "https://hm.baidu.com/hm.static?a0e66ced62f1926ee48b5f059ad9f039";
        ...
        if (curProtocol === 'https') {
            bp.src = 'https://zz.bdstatic.com/linksubmit/push.static';
        }
        else {
            bp.src = 'http://push.zhanzhang.baidu.com/push.static';
        }
        var s = document.getElementsByTagName("script")[0];
        s.parentNode.insertBefore(bp, s);

        var src = (document.location.protocol == "http:") ? "http://static.passport.qihucdn.com/11.0.1.static?e9003f5cb9108d8187ade032b2898cc3" : "https://jspassport.ssl.qhimg.com/11.0.1.static?e9003f5cb9108d8187ade032b2898cc3";
            ...
            if (response.error !== 1) {
                ...
                if (response.button.name) {
                    if (response.button.upgrade) {  //能升级的
                        if (response.isAllVip) {
                            buttonHtml = '<div class="upvip"><a href="' + response.button.url + '"target="_blank" onclick="entrance(57);buttonClickRecord(70);"><img src="//static/assets/revision/images/upvip2.png"></a></div>'
                        } else {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.error !== 1) {
                ...
                systemHtml += '<div class="news-con pl news-con-comment" style="display: none;"><div class="loading"><img src="https://static/assets/revision/images/nav/hk_loading.gif"></div></div>' +
                    '<div class="news-con pl news-con-lesson" style="display: none;"><div class="loading"><img src="https://static/assets/revision/images/nav/hk_loading.gif"></div></div>';
...
function load() {
    ...
    (function () {
        ...
        stat.async = true;
        stat.src = 'https://stats.hanmaker.com/stats.static?v=2.0';
        var s = document.getElementsByTagName('script')[0];
 ...
 */
function open_kefu() {
    window.open('http://wpa.b.qq.com/cgi/wpa.php?ln=2&uin=800190846&eid=218808P8z8p8y8z8R8x8p&o=www.yihao86&q=7&ref=' + document.location, '_blank', 'height=544, width=644,toolbar=no,scrollbars=no,menubar=no,status=no');
}
...
/* COMBO: filename = static/pgc/main.v4.static, type = application/javascript, resp = 200 */
...
function ckXPlayHandler() {
    window.ckPlayerPlayCallback();
}

//ck暂停播放
function ckXPauseHandler() {
    window.ckPlayerPauseCallback();
}
...
(function ($, window, document, undefined) {
    var Player = function (ele, opt) {
        ...
        this.defaults = {
            'playerWidth': '920',
            'playerHeight': '562',
            'playerHtml5Dom': '<video id="html5-player" class="video-static vjs-default-skin vjs-big-play-centered"></video>',
            'playerFlashDom': '<div id="flash-player" style="width:100%;height:100%"></div>',
            'playerVideoUrl': '',
            'timing': false,  //定时执行
            'currentTime': false,  //设置当前视频时间
            'initialization': false,  //播放器初始化完成后执行函数
            'play': false,    //视频播放事件
            'ended': false,    //视频播放结束事件
            'seeking': false,    //视频播放正在快进/后退
            'seeked': false,    //视频播放快进/后退完成
            'pause': false   //视频暂停事件
        };
    ...
    Player.prototype = {
        //创建不同播放器初始化播放插件对象
        createVideoDom: function (type) {
            var _this = this;
            if (type === 'html5') {
                _this.$element.prepend(_this.options.playerHtml5Dom);
            } else {
                _this.$element.prepend(_this.options.playerFlashDom);
            }
        ...
        playByHtml5: function () {
            ...
            var options = {
                ...
                preload: 'auto',
                controls: true,
                file: _this.options.playerVideoUrl,
                type: 'hls',
                width: '1000px',
                height: '900px'
            };
            var player = new QiniuPlayer('html5-player', options, function () {
                //初始化完成后执行的函数
                if ((typeof _this.options.initialization) === 'function') {
                    _this.options.initialization();
                }
                var engine = player.engine();
                if (engine === 'Html5') {
                    //先删除大按钮
                    $('button.vjs-big-play-button').remove();
                    var currentPlaybackRate = player.playRate();
                    var str =
                ...
                //绑定播放事件
                if ((typeof _this.options.play) === 'function') {
                    player.on("play", _this.options.play);
                }

                //绑定暂停事件
                if ((typeof _this.options.pause) === 'function') {
                    player.on("pause", _this.options.pause);
                }

                //绑定快退事件
                if ((typeof _this.options.seeking) === 'function') {
                    player.on("seeking", _this.options.seeking);
                }

                //绑定快进事件
                if ((typeof _this.options.seeked) === 'function') {
                    player.on("seeked", _this.options.seeked);
                }

                //设置播放时间
                if ((typeof _this.options.currentTime) === 'number') {
                    player.currentTime([_this.options.currentTime]);
                }

                //绑定播放结束事件
                if ((typeof _this.options.ended) === 'function') {
                    player.on("ended", _this.options.ended);
                }

                player.on("timeupdate", function () {
                    //设置播放时间
                    if ((typeof _this.options.timing) === 'object') {
                        var time = _this.options.timing[0];
                        var func = _this.options.timing[1];
                        var currentTime = player.currentTime();
                        currentTime = parseInt(currentTime);

                        if (typeof _this.currentTime === 'undefined') {
                            _this.currentTime = currentTime;
                        }
                        if (currentTime !== 0 && (currentTime % time) === 0 && currentTime !== _this.currentTime) {
                            _this.currentTime = currentTime;
                            func(_this);
                        }
                    }
        ...
        init: function () {
            var _this = this;
            var checkPlayerType = _this.selectPlayer();
            if (Param.videoUseFlash === '1' || checkPlayerType === false) {
                _this.createVideoDom('flash').playByFlash();
            } else {
                _this.createVideoDom('html5').playByHtml5();
...
//视频播放
(function ($) {
    var videoContainer = $('div.video-img');
    videoContainer.one('click', function () {
        if (Param.isLogin == 0) {
            toLogin();
            return false;
        }
        $.ajax({
            url: Url.videoPlay,
            data: {id: Param.id, "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")},
            method: 'post',
            'success': function (response) {
                ...
                if (response.error === 0) {
                    ...
                    // videoPlay('http://og9dz2jqu.cvoda.com/Zmlyc3R2b2RiOldlYiBEZXZlbG9wbWVudCBJbiAyMDE3IC0gQSBQcmFjdGljYWwgR3VpZGUtOWhES2ZCS3VYakkubWt2_q00000001.m3u8', parseInt(response.time));
                    videoPlay(response.url, parseInt(response.time));
                } else {
                    payPopup._init();
                    $(".pay-main").removeClass('hide');
                }
            }
        });

        function videoPlay(url, time) {

            $('#huke88-video-play').remove();
            videoContainer.hkPlayer({
                'playerVideoUrl': url,  //播放视频源地址
                'currentTime': time,  //设置开始播放时间

                'timing': [5, function (player) {  //设置定时执行方法
                    var getCurrentTime = player.getCurrentTime();  //当前播放时间
                    ajaxVideoTime(getCurrentTime, 0);
                }],  //设置当前视频时间

                'initialization': function () {  //播放器加 载完成后执行方法

                },
                'error': function () {  //播放器错误，执行方法
                    alert('播放失败，请联系客服人员');
                },
                'play': function () {    //开始播放，执行方法
                    // console.log('开始播放');
                },
                'pause': function () {  //播放暂停，执行方法
                    // ajaxVideoTime(player.getCurrentTime(), 0);
                },
                'ended': function () {  //播放结束，执行方法
                    ajaxVideoTime(0, 1);
                    $(".next-class").removeClass('hide');
                },
                'seeking': function () {  //视频播放正在快进/后退
                    console.log('视频播放正在快进');
                },
                'seeked': function () {  //视频播放快进/后退完成
                    console.log('视频播放快进/后退完成');
                }
            });
        }

        function ajaxVideoTime(time, is_end) {
            $.ajax({
                type: "post",
                url: Url.videoPlayTime,
                data: {
                    id: Param.id,
                    time: time,
                    is_end: is_end,
...
//宣传片视频播放
(function ($) {
    var videoContainer = $('div.video-detail-img');
    ...
    videoContainer.one('click', function () {
        ...
        $.ajax({
            url: Url.trailerVideoPlay,
            data: {uid: Param.uid, id: Param.pgc_id, "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")},
            method: 'get',
            'success': function (response) {
                response = JSON.parse(response);
                ...
                if (response.error === 0) {
                    // $(".video-img").addClass('hide');
                    // trailerVideoPlay('http://og9dz2jqu.cvoda.com/Zmlyc3R2b2RiOldlYiBEZXZlbG9wbWVudCBJbiAyMDE3IC0gQSBQcmFjdGljYWwgR3VpZGUtOWhES2ZCS3VYakkubWt2_q00000001.m3u8');
                    trailerVideoPlay(response.url);
                } else {
                    alert('播放失败，请刷新页面重试或联系客服人员');
                }
            }
        });

        function trailerVideoPlay(url) {

            $('#huke88-video-play').remove();

            videoContainer.hkPlayer({
                'playerVideoUrl': url,  //播放视频源地址
                'currentTime': 0,  //设置开始播放时间
                'initialization': function () {  //播放器加 载完成后执行方法
                    $('#huke88-video-play').remove();
                },
                'error': function () {  //播放器错误，执行方法
...
var pgcDownload = {
    ...
    'ajax': function () {
        $.ajax({
            url: Url.ajaxDownload,
            data: {pgc_id: Param.pgc_id, "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")},
            type: 'json',
            method: 'get',
            'success': function (response) {
                response = JSON.parse(response);
                if (response.error === 0) {
                    pgcDownload.download(response.url);
                }
            }
        });
    },
...
;(function ($, window) {
    ...
    $.fn.pgc = function (options) {
        ...
        this.oConfig = $.extend(defaults, options);
        ...
        var collect = function () {
            ...
            $.ajax({
                url: _oself.oConfig.url,
                data: _oself.oConfig.data,
                type: 'POST',
                dataType: "json",
                success: function (response) {
                    if (response.error === 0) {

                        $(".collect-yes,.collect-cancel").attr('data-status', 1);

                        if ($(".collect-yes").hasClass('hide')) {
                            $(".collect-yes").removeClass('hide');
                            $(".collect-cancel").addClass('hide');
                        } else {
                            $(".collect-yes").addClass('hide');
                            $(".collect-cancel").removeClass('hide');
                        }
                    }
                },
                error: function () {
...
var pgc = $("body").pgc(collecConfig);

$(".collect-yes, .collect-cancel").on("click", function () {
...
$(".collect-cancel").on('mouseover', function () {
    $(this).html('取消收藏');
});

$(".collect-cancel").on('mouseout', function () {
    $(this).html('已收藏');
...
var pay = {
    ...
    alipay: function () {
        ...
        if (Param.status === 1) {
            ...
            $.ajax({
                type: 'post',
                url: pay.config.alipay.url,
                data: pay.config.alipay.data,
                dataType: 'json',
                success: function (message) {
                    if (message.isError === 0) {
    ...
    wechat: function () {
        ...
        $.ajax({
            type: 'post',
            url: pay.config.wechat.url,
            data: pay.config.wechat.data,
            dataType: 'json',
            success: function (message) {
                ...
                if (message.isError === 0) {
                    ...
                    if (0 == message.isError) {
                        clearInterval(Param.ds);
                        $('#wxpay').html('<img style="height:175px;width: 175px;" src="' + message.codeImgUrl + '">');

                        if (Param.flag < 300) {
                            Param.ds = setInterval(function () {
                                ajaxWxCallback(message.order_no, Param.flag);
                                Param.flag = Param.flag + 2;
                            }, 3e3);
                        } else if (Param.flag >= 300 && Param.flag < 600) {
                            Param.ds = setInterval(function () {
                                ajaxWxCallback(message.order_no, Param.flag);
                                Param.flag = Param.flag + 2;
                            }, 1e4);
        ...
        function ajaxWxCallback(order_no) {
            if (Param.flag >= 600) {
                clearInterval(Param.ds);
            }
            $.ajax({
                type: "post",
                url: pay.config.wechat.afterUrl,
                data: {
                    order_no: order_no,
                    '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')
                },
                ...
                success: function (message) {
                    if (message.isError == 0) {
                        ...
                        $(".pay-main").addClass('hide');
                        $(".pay-success").removeClass('hide');
                    }
                }
            });
        }
...
var payPopup = {
    ...
    _init: function () {
        ...
        //关闭弹窗
        $('.pgc-plug').on('click', '.close', function () {
            $(this).parents('.pay-win').addClass('hide');
            /* Act on the event */
            payPopup.wechatClose();
        });
    ...
    alipay: function () {
        ...
        if (Param.status === 1) {
            ...
            $.ajax({
                type: 'post',
                url: payPopup.config.alipay.url,
                data: payPopup.config.alipay.data,
                dataType: 'json',
                success: function (message) {
                    if (message.isError === 0) {
                        window.location.href = message.link;
    ...
    wechat: function () {
        ...
        function transferInfo() {
            var $container = $(".pay-main .info-m");
            var img = $container.find(".info-img").attr('src');
            var txt = $container.find(".txt").html();

            var $newContainer = $(".pay-success .info-m");
            $newContainer.find(".info-img").attr('src', img);
            $newContainer.find(".txt").html(txt);
        }
        ...
        $.ajax({
            type: 'post',
            url: payPopup.config.wechat.url,
            data: payPopup.config.wechat.data,
            dataType: 'json',
            success: function (message) {
        ...
        function ajaxWxCallback(order_no) {
            ...
            $.ajax({
                type: "post",
                url: payPopup.config.wechat.afterUrl,
                data: {
                    order_no: order_no,
                    '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    $.ajax({
        ...
        success: function (message) {
            if (message.error === 0) {
                ...
                switch (message.code) {
                    case 0:
                        html = '<a href="' + message.url + '" class="goon fl pos" target="_blank">立即学习</a>';
                        break;
                    case 1:
                        html = '<a href="' + message.url + '" class="goon fl pos" target="_blank">继续学习</a>';
                        break;
                    case 2:
                        html = '<a href="' + message.url + '" class="re fl pos" target="_blank">在学一次</a>';
                        break;
                }
...
// 刷新学习社区列表
var refreshCommunity = function (_page) {


    if ($(".t3").hasClass('loaded') === false) {
        $(".t3").addClass('loaded');
    }

    //加载列表
    var pages = parseInt(_page) || 0;
    $.ajax({
        type: "get",
        url: Url.studyCommunity,
        data: {
            pgc_id: Param.pgc_id,
            pages: pages
        },
        ...
        success: function (message) {
            ...
            if (message.error === 0) {
                ...
                $.each(message.data.list, function (k, v) {
                    ul +=
                        '<div class="list fl">' +
                        '<a href="' + v.url + '" target="_blank"><img src="' + v.avator + '"></a>' +
                        '<p><a href="' + v.url + '" target="_blank">' + v.username + '</a></p>' +
                        '</div>';
                });

                //总数
                $(".t3 .sjqun .study-community-number").html(message.data.count);

                //列表
                $(".t3 .sj-list").html(ul);

                //分页
                $(".t3 #page-m").html(message.data.html);


                //分页跳转绑定事件
                $(".t3 #page-m .page_click_class").on('click', function () {
                    refreshCommunity($(this).attr('data-page'));
                });
            }
        }
    });
...
$(".study-community").on('click', function () {
    //是否已经加载过
    if ($(".t3").hasClass('loaded')) {
        return false;
    }
    refreshCommunity();
});
...
// 支付页面初始化
if (Param.page_type === 'pay') {
    pay._init();
}

// 视频详情页购买弹窗（初始化支付）
$(".buy-class").on('click', function () {
    if (Param.isLogin == 0) {
        toLogin();
        return false;
    }
...
// 课程详情页tab切换
$('.info-m-l').on('click', '.head a', function () {
    var n = $(this).index();
    $(this).addClass('active').siblings().removeClass('active');
    $('.info-m-l .tab-m .t').eq(n).removeClass('hide').siblings().addClass('hide');
});
...
// 视频播放页分享弹出
$('.video-b').on('click', '.down-icon', function (e) {
    $('.video-b .btn-m').show();
    e.stopPropagation();
});
...
// 目录的列表鼠标滑过
$(".pgc-info-m .info-m-l .t2 .list").on('mouseover', function () {
    if ($(this).attr('data-status') === '1') {
        studyPgc($(this));
    }
});
...
$(window).scroll(
    function () {
        ...
        if ((scrollHeight - scrollTop - windowHeight) <= 300) {
            ...
            //是否在目录标签
            var loading = $(".pgc-info-m .loading-icon");
            var end = $(".pgc-info-m .no-more");

            var isCatalog = $(".pgc-catalog").hasClass('active');
            var isNotEnd = end.hasClass('hide');
            var status = loading.attr('data-status');


            if (isCatalog && isNotEnd && status == 1) {

                loading.removeClass('hide');
                loading.attr('data-status', 0);

                $.ajax({
                    type: 'GET',
                    url: Url.ajaxGetPgcCatagory,
                    data: {page: Param.page, id: Param.pgc_id},
                    dataType: 'JSON',
                    success: function (data) {
                        ...
                        if (data.end) {
                            ...
                            $.each(data.list, function (k, v) {
                                ...
                                html += '' +
                                    '<div class="list" data-id="' + v.id + '" data-status="1">' +
                                    '<i class="fl play-icon"></i>' +
                                    '<a href="' + v.lookUrl + '" class="class-name fl" target="_blank">课时' + ( (data.page - 1) * Param.limit + 1 + k ) + '：' + v.title + '</a>' +
                                    '<span class="fr time">' +
                                    '<i class="time-icon"></i>' + v.duration + '</span>' +
                                    '</div>';
                            });
...
/* COMBO: filename = static/681.static, type = text/plain, resp = 404 */
