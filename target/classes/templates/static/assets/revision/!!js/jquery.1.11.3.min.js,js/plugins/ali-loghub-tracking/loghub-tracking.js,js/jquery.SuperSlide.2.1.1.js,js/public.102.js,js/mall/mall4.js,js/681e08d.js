...
/* COMBO: filename = static/mall/mall4.static, type = application/javascript, resp = 200 */
/*
* 商城所有js
***/

/*****************************************1. param start*************************************************/
var $container = $("#mall-win");
/*****************************************1. param end*************************************************/

/*商城公共**/
$('.task-state.signin').click(function (e) {
    var layer = $("#sign-win");
    layer.removeClass('hide');
    getSignData();
    stopPropagationMall(e);
})

function stopPropagationMall(e) {
    if (e.stopPropagation) {
        e.stopPropagation();
    } else {
        e.cancelBubble = false;
...
// 选择购买型号切换
$(".buy-wrap").on("click", ".xh .input-r label", function () {
...
// 购买数量加减
$(".num-btn").on("click", ".add", function () {
    return false;
    if (Param.surplus == 0) {
        return false;
    }
    var t = $(this).siblings(".num-con");

    b = $(".surplus-num span").html();
    ...
    setTotal(parseInt(t.val()));
});

//数量加号
$(".num-btn").on("click", ".minus", function () {
    return false;
    if (Param.surplus == 0) {
        return false;
    }
...
$(".cancel").on('click', function () {
    $("#mall-win").addClass("hide");
    $(this).closest(".win-m").addClass("hide");
});


// 购买按钮
$(".buy-btn .buy-btn-a").on('click', function () {
    if (Param.isLogin == 0) {
        toLogin();
        return false;
    }
    ...
    var title = $(".buy-wrap .buy-r .name").text();

    if (parseInt(price) > parseInt(Param.self_price)) {
        $(".fail .main .p2 span").html(parseInt(price - Param.self_price));
        $(".fail").removeClass('hide');
        return false;
    }
    if (Param.type == 1 || Param.type == 3 || Param.type == 4) {

        //虚拟物品
        var purch = $(".purch");
        purch.find(".p2").html(
            '您将使用<span>' + price + '</span>虎课币兑换' + title
        );
        purch.removeClass('hide');
    } else {
        //实物
        if (Param.hasAddress == 0) {
            //无收获地址
            ...
            var good = $("tbody");
            var num = $(".num-con").val();
            var img = $(".buy-l").attr('data-img');
            good.find(".gold").html(Param.unit_price);
            good.find(".num").html('数量:' + num);
            $(".dxnum .gold").html(price);
            good.find(".title").html(title);
            good.find(".img").attr('src', img);
            $container.find(".firm-order").removeClass('hide');
        }
    }
});

//添加收货地址
$(".ojbkt").on('click', function () {
    var form = $("#editAddress");

    //验证收货人
    var name = form.find('input[name=name]').val();
    if (name == '') {
        form.find('input[name=name]').closest('.input-list').addClass("error");
        return false;
    }
    ...
    if (!checkTelNum(phone)) {
        form.find('input[name=phone]').closest('.input-list').addClass('error');
        return false;
    }
    ...
    var region = form.find('select[name=region]').val();
    if (province == '省份' || province == '') {
        form.find('select[name=province]').closest('.input-list').addClass('error');
        return false;
    }
    ...
    if (address == '') {
        form.find('textarea[name=address]').closest('.input-list').addClass("error");
        return false;
    }
    var price = parseInt($(".goods_price").text());
    var title = $(".buy-wrap .buy-r .name").text();
...
//确认购买按钮
$(".purch .btnm .btn").on('click', function () {
        ...
        success: function (data) {
            ...
            if (data.error == '0') {
                if (Param.type == 1) {
                    ...
                    var $item = $(".successful-coupon");
                } else if (Param.type == 4) {
                    var $item = $(".successful-sucai");
                    $item.find('input').val(data.message);
                }

                $item.find('p2').html('所购商品：' + $(".buy-wrap .buy-r .name").text());
                $item.removeClass('hide');
            } else {
...
//确认购买按钮
$(".firm-order .sbtn .btn").on('click', function () {

    var model_type = '';
    if (Param.type == 2) {
        model_type = $(".model_type_parents .active").attr("data-type");
        // if(model_type == 'und')
    }
        ...
        success: function (data) {
            ...
            if (data.error == '0') {
                if (Param.type == 1) {
                    ...
                    $(".firm-order").addClass('hide');
                    var $item = $(".successful-shiwu");
                    $item.find('.p2').html('所购商品：' + $(".buy-wrap .buy-r .name").text());
                }
                $item.removeClass('hide');
            } else {
...
//充值成功关闭弹窗
$container.find(".close-success").on('click', function () {
    window.location.reload();
});
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.error === 0) {
                ...
                $(".error-messgae").removeClass('hide');
            }
        },
        error: function () {
            console.log('请联系管理员');
...
function clearError($this) {
    $this.closest(".input-list").removeClass('error');
}

//管理收货地址弹窗
$(".mall-t").on("click", ".dz", function () {
    if (Param.hasAddress !== '0') {
        $("#mall-win,#mall-win .address").removeClass("hide");
    } else {
        $("#mall-win,#mall-win .no-address").removeClass("hide");
    }
});

//管理收货地址弹窗
$(".user-box").on("click", ".dz", function () {
    if (Param.hasAddress !== '0') {
        $("#mall-win,#mall-win .address").removeClass("hide");
    } else {
        $("#mall-win,#mall-win .no-address").removeClass("hide");
...
$(".add-address").on("click", function () {
    addressClear();
    $(this).closest('.win-m').addClass("hide");
    $("#mall-win .add").removeClass("hide");
});

//function-删除收获地址
function addressDel(container) {
    $.ajax({
        type: "post",
        url: Url.addressDel,
        data: {id: container.attr('data-id'), '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')},
        dataType: "json",
        success: function (data) {
            if (data.error == '0') {
                container.remove();
...
//function-修改地址
function addressEdit(container) {

    //获取信息
    var name = container.find(".name").html(),
        phone = container.find(".phone").html(),
        $address = container.find(".province"),
        is_default = container.attr("data-default"),
        id = container.attr('data-id');


    var province = $address.attr('data-province'),
        city = $address.attr('data-city'),
        region = $address.attr('data-region'),
        detail_address = $address.attr('data-detail_address');

    //切换model
    $("#mall-win .address").addClass("hide");
    $("#mall-win .add").removeClass("hide");
    ...
    change(0);
    $form.find('select[name=province]').val(province);
    change(1);
    $form.find('select[name=city]').val(city);
    change(2);
    $form.find('select[name=region]').val(region);

    $form.find('textarea[name=address]').val(detail_address);

    if (is_default == 1) {
        $(".radio").trigger('click');
        // $form.find('.radio').attr("checked",true);
    }

    $form.find('input[name=id]').val(id);

    //更换按钮
    $(".win-m .cancel").addClass('hide');
    $(".win-m .back").removeClass('hide');
}

//onclick-删除地址
$(".win-m .address-list .opera-btn .del").on('click', function () {
    var container = $(this).closest('.address-list');
    addressDel(container);
});


//编辑地址
$(".win-m .address-list .opera-btn .edit").on('click', function () {
    var container = $(this).closest('.address-list');
    addressClear();
 ...
 * 编辑地址框返回按钮
 */
$(".win-m .back").on('click', function () {
    $(this).closest('.win-m').addClass('hide');
    $("#mall-win .address").removeClass("hide");
});
...
function addressClear() {
    var container = $("#mall-win .add");
    container.find('input[type=text]').val('');
    container.find('input[name=default]').removeAttr('checked');
    container.find('textarea').val('');
    container.find('select[name=province]').val('省份');
    container.find('select[name=city]').val('地级市');
    container.find('select[name=region]').val('市、县级市');
}
...
// 我要晒单弹窗
$(".thesun").on("click", ".nosd .btn,.wantsd a", function () {
    ...
    $("#mall-win,#mall-win .upload-sd").removeClass("hide");
});


//创建晒单
function createUploader(res) {
    var uploader,
        $message = $(".message"),
        $pic = $("#upbtn");
    ...
    uploader.on('uploadSuccess', function (file, response) {
        ...
        if ($(".img-show .imglist").length < 3) {
            var $li = '<div class="imglist fl" data-img="' + response.url + '" style="background-image: url(//imgs/' + response.url + '!/both/250x250);"><i class="del"></i></div>';
            $('#upbtn').before($li);

            var pic = $("#uploadComment .picture").val();
            if (pic == '') {
                $("#uploadComment .picture").val(response.url);
            } else {
                $("#uploadComment .picture").val(pic + '+' + response.url);
            }
        }

        if ($(".img-show .imglist").length >= 3) {
            $(".webuploader-pick").html('最多3张');
        }
    ...
    uploader.on('uploadComplete', function (file) {
        $message.html(' ');
        // $('.webuploader-pick').parent('div').remove();
    });

    //上传失败
    uploader.on('uploadError', function (file, reason) {
        if (reason === 'http') {
            layer.alert('上传速度太快了，休息一会');
        } else {
            layer.alert('上传出错');
        }

        $.ajax({
            url: Url.uploadSignUrl,
            dataType: 'JSON',
            async: true,
            type: 'GET',
            success: function (res) {
    ...
    //删除文件
    $(".img-show .imglist .del").on('click', function () {
        var $pic = $("#uploadComment .picture"),
            val = $(this).closest('.imglist').attr('data-img'),
            $newPic = '';
        $(this).closest('.imglist').remove();


        $.each($pic.val().split('+'), function (k, v) {
            if (v != val) {
                if ($newPic == '') {
                    $newPic = v;
                } else {
        ...
        uploader.refresh();
        if ($(".imglists").children().length < 3) {
            $(".webuploader-pick").html('点击上传图片');
        }
    });
}
...
//提交晒单请求
$(".upload-sd").on('click', '.tj', function () {

    var $form = $("#uploadComment"),
        $comment = $form.find('textarea'),
        $picture = $form.find('.picture');
    ...
    if (comment == '' || picture == '') {
        ...
        if (picture == '') {
            ...
            $(".message").html('至少上传一张图片').css('color', 'red');
        }
        return false;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error === 0) {
                $(".upload-sd"), $("#mall-win").addClass('hide');
                window.location.reload();
            }
        },
        error: function () {
...
$("textarea").on('focus', function () {
    $("#uploadComment textarea").css('border', '1px #e8e8e8 solid');
    $(".message").html(' ').css('color', '#666');
})

// 通用关闭弹窗关闭
$("#mall-win").on("click", ".close", function () {
    $(this).parents(".win-m").addClass("hide");
    $("#mall-win").addClass("hide");
});

// 浏览晒单图片
$("#hmall .evaluate-list").on("click", ".list-r", function () {
    var $container = $(".evaluate-list-m");
    var list = $(this).closest('.list');

    var left = list.find('.list-l').html();

    $container.find('.list-l').html(left);
    ...
    $.each(arr, function (k, v) {
        html += '<li class="slide-item" style="background-image: url(' + v + ')"><span class="num">图' + (k + 1) + '</span></li>';
    });
    $container.find(".list-r ul").html(html);
    ...
    $("#mall-win,#mall-win .evaluate-list").removeClass("hide");
});
...
/*****************************************5 .其他JS start*************************************************/
//立即获取虎课币
$container.find(".get_gold").on('click', function () {
    window.open(Param.taskUrl);
});
...
function onInputs(_self) {
    $(_self).closest('#uploadComment').find('em').text($(_self).val().length);
}
