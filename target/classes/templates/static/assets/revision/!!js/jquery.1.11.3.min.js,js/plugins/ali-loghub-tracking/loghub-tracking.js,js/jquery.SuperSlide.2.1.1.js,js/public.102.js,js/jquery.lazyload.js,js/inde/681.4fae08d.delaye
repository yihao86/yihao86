...
/* COMBO: filename = js/category/c4d.v2.js, type = application/javascript, resp = 200 */
$(function () {
    $('.lazy').lazyload({threshold: 200});
    var csrfToken = $('meta[name="csrf-token"]').attr("content");

    $(".list-t").on("click",".tit-nav a",function(){
        // 如果标签已经被选择则不触发后续操作
        ...
        var videoClass = 6; // c4d
        var contId = '#cont-' + type;
        var moreId = '#more-' + type;

        $(this).addClass("active").siblings().removeClass("active"); // 改变标签状态
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // 改变查看更多按钮链接
                if (type == 'recommend') {
                    $(moreId).find('a').attr('href', '/c4d/6-0-0-0-' + $res.difficulty + '-2-0/');
                } else {
                    $(moreId).find('a').attr('href', '/c4d/6-0-0-0-' + $res.difficulty + '-1-0/');
                }
