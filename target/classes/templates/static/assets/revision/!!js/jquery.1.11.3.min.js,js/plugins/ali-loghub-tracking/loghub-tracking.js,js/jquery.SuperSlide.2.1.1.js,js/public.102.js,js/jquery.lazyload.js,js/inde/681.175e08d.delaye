...
/* COMBO: filename = js/video-list/list.js, type = application/javascript, resp = 200 */
/**
 * Created by Mr.D on 2017/6/13.
 */
...
$(function () {
    ...
    // 查看更多(思路:获取div的展示宽度,  遍历标签宽度,如果宽度超过,就显示)
    var $tagListDiv = $('div.limit-height > div.tiny-item');
    $.each($tagListDiv, function (k, v) {
        var $width = $(this).width();

        var $tagListA = $(this).find('div.tiny-list > a');
        var $tagListAWidth = 0;
        $.each($tagListA, function (kk, vv) {
            $tagListAWidth += $(this).width() + 32;
        });

        if ($tagListAWidth > $width) {
            $(this).find('span.tiny-more').removeClass('hide');
            $('span.tiny-more').off('click').on('click', function () {
                if (!$(this).hasClass('check')) {
                    $(this).addClass('check');
                    $(this).html('<i></i>收起');
                    $(this).siblings('.tiny-list').css({
                        height: 'auto',
                        'overflow': 'visible'
                    })
                } else {
    ...
    // 难度切换
    $('.sort-tags .difficulty').on("click", function () {
        var $upUrl = $(this).attr('data-bind-up');
        var $downUrl = $(this).attr('data-bind-down');
     ...
     * 列表页 标签点击次数统计
     */
    $('div.tags-list-click a').bind('click', function () {
        var $this = $(this);
