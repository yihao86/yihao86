...
/* COMBO: filename = js/lesson/aggregate.js, type = application/javascript, resp = 200 */
//****************************************系列课首页****************************************//
function fixedLesson(){
    var scrollTop = $(document).scrollTop();
    fm        = $(".class-list").find(".class-nav");
    documentHeight = $(document).height();
    windowHeight = $(window).height();
    contentItems = $(".class-list").find(".class-item");
    currentItem = "";
    ...
    if (currentItem != $(".class-nav").find(".active").attr("href")) {
        $(".class-nav").find(".active").removeClass("active");
        $(".class-nav").find("a[href='" + currentItem + "']").addClass("active");
    }
    if(scrollTop>755) {
        fm.addClass("fixed");
    }
    else {
        fm.removeClass("fixed");
    }
}
...
$("#collect").on('click', function () {
    toLogin();
    $.post(Url.lessonCollect, {
        "PersonalCollectionFrom[video_id]": Param.lessonId,
        "type": 2,
        "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")
    }, function (b) {
        0 == b.isError ? ($("#collect").addClass("hide"), $("#cancel-collect").removeClass("hide"), $("#collected-video").html('已收藏')) : ''
    }, "json")
});

//取消收藏点击hover
$('#cancel-collect').hover(function () {
    $(this).html('取消');
}, function () {
    $(this).html('已收藏');
});
...
$("#cancel-collect").on('click', function () {
    toLogin();
    $.post(Url.lessonCancelCollect, {
        "PersonalCollectionFrom[video_id]": Param.lessonId,
        "type": 2,
        "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")
    }, function (b) {
        0 == b.isError ? ($("#cancel-collect").addClass("hide"), $("#collect").removeClass("hide"), $("#collected-video").html('取消')) : ''
    }, "json")
});


exposure.type = 2;
exposure.maxLength = 3;
exposure._init();
