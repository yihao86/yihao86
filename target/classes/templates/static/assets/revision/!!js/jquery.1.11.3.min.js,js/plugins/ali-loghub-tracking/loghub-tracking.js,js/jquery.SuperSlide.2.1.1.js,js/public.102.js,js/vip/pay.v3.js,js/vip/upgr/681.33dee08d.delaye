...
/* COMBO: filename = js/vip/pay.v3.js, type = application/javascript, resp = 200 */
/**
 * 支付插件
 */
;(function($) {
...
/* COMBO: filename = js/vip/upgrade.v.2.js, type = application/javascript, resp = 200 */
/**
 * 分成两个三个模块
 * 1.事件触发dom变化
 * 2.根据dom确定充值参数
...
$(function () {
    ...
    var ddd = {
        ...
        data : {
            'container': $(".choose"),
            'wxContainer': $(".scan-qr-code .m .wechat .code-img"),
            'aliContainer': $(".scan-qr-code .m .ali"),
            'ordersContainer': $(".my-vip .vip-list"),
            'priceContainer': $(".scan-qr-code .t"),
            'typeInput': $("input[name=type]"),
            'moneyInput': $("input[name=money]"),
        ...
        init:function () {
            ...
            //TODO:页面调整
            if(Param.justLifelong === '1'){
                $(".vip-info .main .choose:eq(0)").addClass('hide');
                $(".vip-info .main .choose:eq(1)").trigger('click');
            }else{
                $(".vip-info .main .choose:eq(0)").trigger('click');
            }
        },
        ...
        init_pay: function () {
            ...
            this.data.pay = new payPlugin({
                ...
                wxPayContainer: this.data.wxContainer,
                csrf: this.data.csrf.attr('content')
            });

            //绑定支付宝点击事件
            this.data.aliContainer.click(function () {
        ...
        getData: function () {
            return {
                type: this.data.typeInput.val(),
                money: this.data.moneyInput.val(),
        ...
        change: function () {
            ...
            this.data.container.on('click', function () {
        ...
        //渲染页面参数
        changeDom: function ($this) {

            $(".vip-info .main .active").removeClass('active');
            $this.addClass('active');


            var price = $this.attr('data-price'),
                type = $this.attr('data-type'),
                lifelong = $this.attr('data-lifelone');

            var unitPrice = (price / 365).toFixed(2);

            var priceHtml = '';
            if(lifelong === '1'){
                this.data.ordersContainer.find('.forever-identity').removeClass('forever not');
                this.data.ordersContainer.find('.can-up-identity').removeClass('hide');
                this.data.ordersContainer.find('.tips').addClass('hide');
                priceHtml = '付款金额：<span><i class="price-data">' + price + '</i>元</span>';
            }else{
                this.data.ordersContainer.find('.forever-identity').addClass('forever not');
                this.data.ordersContainer.find('.can-up-identity').addClass('hide');
                this.data.ordersContainer.find('.tips').removeClass('hide');

                priceHtml = '付款金额：<span><i class="price-data">' + price + '</i>元/年</span>';
                if(unitPrice !== '0.00'){
                    priceHtml += '低至约<span><i class="day-p">' + (price / 365).toFixed(2) + '</i>元/天</span>';
                }
            }

            this.data.priceContainer.html(priceHtml);
            this.data.typeInput.val(type);
            this.data.moneyInput.val(price);

            //更新支付状态
            if(Param.isLogin !== '0'){
                this.data.pay.call(ddd.getData(), 'wx', Url.successUrl);
            }
        }
    };
...
//统计页面到达
$.post(Url.record_pv_uv_ip, {uv_id: Param.uv_id, model_type: 12, uid: Param.uid},
    function () {
