...
/* COMBO: filename = static/vip/all-vip-pay.v13.static, type = application/javascript, resp = 200 */
$(function () {
    init();
    checkDomPrice();
    wxpayCode();
    ...
    function init() {
        var voucher_discount = $(".voucher_discount");
        //被禁止的优惠券价格
        var maxDis = 0;
        var hasChoice = false;
        $.each(voucher_discount.find('option'), function (k, v) {
            if (parseInt($(this).val()) > maxDis) {
                maxDis = parseInt($(this).val());
            }
            hasChoice = true;
        });
        $(".voucher_discount").val(maxDis);
    }

    //虎课币
    $(".layout-cont").on('click', ".huke-ratio .ratio-ic-gold", function () {
        var ischeck = $(".ratio-ic-gold").hasClass("active");
        if (ischeck == false) {
            $(".ratio-ic-gold").addClass("active");
            $(".ratio-ic-coupon").removeClass("active");
            $(".ratio-ic-voucher").removeClass("active");
            checkDomPrice();

        } else {
            $(".ratio-ic-gold").removeClass("active");
            checkDomPrice();
    ...
    //优惠券
    $(".layout-cont").on('click', ".huke-ratio .ratio-ic-coupon", function () {
        var ischeck = $(".ratio-ic-coupon").hasClass("active");
        if (ischeck == false) {
            $(".ratio-ic-coupon").addClass("active");
            $(".ratio-ic-gold").removeClass("active");
            $(".ratio-ic-voucher").removeClass("active");
            checkDomPrice();
    ...
    //抵扣券
    $(".layout-cont").on('click', ".huke-ratio .ratio-ic-voucher", function () {
        var ischeck = $(".ratio-ic-voucher").hasClass("active");
        if (ischeck == false) {
            $(".ratio-ic-voucher").addClass("active");
            $(".ratio-ic-gold").removeClass("active");
            $(".ratio-ic-coupon").removeClass("active");
            checkDomPrice();
        } else {
    ...
    //优惠券下拉框
    $(".layout-cont").on('change', ".huke-ratio .conpon_discount", function () {
        checkDomPrice();
        wxpayCode();
    });

    //优惠券下拉框
    $(".layout-cont").on('change', ".huke-ratio .voucher_discount", function () {

        var is_voucher_check = $(".ratio-ic-voucher").hasClass("active");
        if (is_voucher_check) {
            checkDomPrice();
            wxpayCode();
        } else {
            var price = parseInt($(".voucher_discount").val());
            $(".can-use-voucher-price").html(price);
        }
        ...
        if (1 == Param.status) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    useVoucher: getUseVoucher(),
                    come_host: Param.come_host,
                    come_type: Param.come_type,
                    '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')
                },
                dataType: 'json',
                success: function (message) {
...
function wxpayCode() {
    ...
    $.post(Url.Wxpay, {
        ...
        if (0 == message.isError) {
            ...
            $('#wxpay').html('<img src="' + message.codeImgUrl + '">');
            if (Param.flag < 300) {
...
//按钮点击处理
function statistics_button_click($vip_type, $button_type, $uid) {
    $.get(Url.button_click_url, {type: $button_type, vip_type: $vip_type, uid: $uid},
        function () {
 ...
 * @param container
 */
function checkDomPrice(container) {

    $this = $(container);

    var price = $this.attr('data-price'),
        unit_price = $this.attr('data-unit-price'),
        unit_gold_price = $this.attr('data-unit-glod-price'),
        gold_price = $this.attr('data-gold-price'),
        gold_limit = $this.attr('data-gold-limit'),
        type = $this.attr('data-type'),
        ischeck = $(".ratio-ic-gold").hasClass("active"),
        is_conpon_check = $(".ratio-ic-coupon").hasClass("active"),
        is_voucher_check = $(".ratio-ic-voucher").hasClass("active");
    ...
    var _price = 0,
        _unit_price = 0,
        //折扣位置的dom
        _deduction_gold = gold_price,
        _deduction_money = gold_limit;

    if (is_conpon_check == true) {
        _price = price * parseInt($(".conpon_discount").val()) / 10;
        _unit_price = (_price / 365).toFixed(2);


    } else if (is_voucher_check == true) {
        _price = price - parseInt($(".voucher_discount").val());
        _unit_price = (_price / 365).toFixed(2);

    } else {
        if (Param.gold >= gold_limit) {
            if (ischeck == true) {
                _price = price - gold_price;
                _unit_price = unit_gold_price;
            } else {
                _price = price;
            ...
            // return false;
            $(".huke-ratio-gold").removeClass('hide');
            $(".huke-ratio-tip").addClass('hide');
        } else if ((gold_limit - 100) > Param.gold) {
            _price = price;
            _unit_price = unit_price;
            $(".huke-ratio-gold").addClass('hide');
            $(".huke-ratio-tip").addClass('hide');
        } else {
            _deduction_gold = _deduction_gold + '元';
            _price = price;
            _unit_price = unit_price;
            $(".huke-ratio-tip").removeClass('hide');
            $(".huke-ratio-gold").addClass('hide');
        }
    ...
    if (type == 9999) {
        $('.crch-pay .pay-tit p span:eq(1)').html('');
        $('.crch-pay .pay-tit p em:first').html(_price + '元');
    } else {
        $('.crch-pay .pay-tit p span:eq(1)').html('低至约' + _unit_price + ' 元/天');
        $('.crch-pay .pay-tit p em:first').html(_price + '元/年');
    }

    if (is_conpon_check == true) {
        $(".can-use-coupon-price").html((price * (1 - parseInt($(".conpon_discount").val()) / 10) ).toFixed(1));
    }


    $(".can-use-gold-price").html(_deduction_gold);
    $(".can-use-gold-limit").html(_deduction_money);

    $("input[name=type]").val(type);
    $("input[name=money]").val(_price);
}
...
function getUseCoupon() {
    var ischeck = $(".ratio-ic-coupon").hasClass("active");
    if (ischeck == true) {
        var discount = $(".conpon_discount").val();
        return parseInt($(".conpon_discount option[value=" + discount + "]").attr('data-id'));
    } else {
        return 0;
    }
}
...
function getUseVoucher() {
    var ischeck = $(".ratio-ic-voucher").hasClass("active");
    if (ischeck == true) {
        var discount = $(".voucher_discount").val();
        return parseInt($(".voucher_discount option[value=" + discount + "]").attr('data-id'));
    } else {
        return 0;
    }
}
...
//统计页面到达
$.post(Url.record_pv_uv_ip, {uv_id: Param.uv_id, model_type: 999, uid: Param.uid},
    function () {
 ...
 * @param container
 */
function checkDomPrice() {
    var isCoupon = $(".ratio-ic-coupon").hasClass('active'),
        isVoucher = $(".ratio-ic-voucher").hasClass('active'),
        isGold = $(".ratio-ic-gold").hasClass('active'),
        conpon_discount_val = $(".conpon_discount").val(),
        voucher_discount_val = $(".voucher_discount").val(),
        _price = Param.price;

    var conpon_discount = parseInt($(".conpon_discount option[value=" + conpon_discount_val + "]").attr('data-id'));
    var voucher_discount = parseInt($(".voucher_discount option[value=" + voucher_discount_val + "]").attr('data-id'));

    if (isCoupon) {
        if (conpon_discount) {
            $(".can-use-coupon-price").html((_price * (1 - conpon_discount_val / 10)).toFixed(1));
            _price = (_price * conpon_discount_val / 10).toFixed(1);
        }

    } else if (isVoucher) {
        if (voucher_discount) {
            $(".can-use-voucher-price").html(voucher_discount_val);
            _price = _price - voucher_discount_val;
        }
    } else if (isGold) {
        _price -= 29;
    }
    ...
    if (Param.type == 9999) {
        $(".price-html").find("em:eq(0)").html(_price + '元');
    } else {
        $(".price-html").find("em:eq(0)").html(_price + '元/年');
        $(".price-html").find("em:eq(1)").html((_price / 365).toFixed(1));
    }
