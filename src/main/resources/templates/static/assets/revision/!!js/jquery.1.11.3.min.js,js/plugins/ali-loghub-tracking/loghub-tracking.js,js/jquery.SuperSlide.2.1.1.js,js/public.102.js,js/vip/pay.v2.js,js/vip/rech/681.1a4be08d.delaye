...
/* COMBO: filename = js/vip/pay.v2.js, type = application/javascript, resp = 200 */
/**
 * 支付插件
 */
;(function($) {
        ...
        'wxpay': function () {
            ...
            $.post(
                ...
                if (0 === response.isError) {
                    ...
                    $('#wxpay').html('<img style="height:175px;width: 175px;" src="' + response.codeImgUrl + '">');

                    if ($this.options.flag < 300) {
                        $this.options.ds = setInterval(function () {
                            $this.wxCallback(response.order_no, $this.options.flag);
                            $this.options.flag = $this.options.flag + 2;
                        }, 3e3);
                    } else if ($this.options.flag >= 300 && $this.options.flag < 600) {
                        $this.options.ds = setInterval(function () {
                            $this.options.flag(response.order_no, $this.options.flag);
                            $this.options.flag = $this.options.flag + 2;
                        }, 1e4);
                    }
                }
...
/* COMBO: filename = js/vip/recharge.v5.js, type = application/javascript, resp = 200 */
/**
 * 分成两个三个模块
 * 1.事件触发dom变化
 * 2.根据dom确定充值参数
 * 3.发起充值
 * Created by gqy on 2018/4/22.
 */
$(function () {
    ...
    var ddd = {
        ...
        //按钮组
        data : {
            'container': $(".crch-intro"),
            'gold': $(".huke-ratio .ratio-ic-gold"),
            'voucher': $(".huke-ratio .ratio-ic-voucher"),
            'coupon': $(".huke-ratio .ratio-ic-coupon")
        },
        ...
        'init':function () {
            ...
            ddd.couponClick();
            ddd.voucherClick();
            ddd.voucherDisCountClick();
            ddd.conponDiscountClick();
            ...
            //点击第二个box
            if( Param.pay_type!='' ){
            	if(Param.pay_type == 'lifelong'){
                    $(".bod-cont:eq(0)").trigger('click');
                }else if(Param.pay_type == 'infinite'){
                	$(".bod-cont:eq(1)").trigger('click');
                }else{
                    $(".bod-cont:eq(2)").trigger('click');
                }
            }else{
            	if(Param.hasAllLife === '1'){
                    $(".bod-cont:eq(1)").trigger('click');
                }else{
                    $(".bod-cont:eq(0)").trigger('click');
                }
        ...
        'init_pay': function () {
            ...
            $('#alipay').click(function () {
                ddd.data.pay.call(ddd.getData(), 'alipay', '');
            });
        },
        ...
        //获取虎课币状态
        'getUseGold': function () {
            var ischeck = ddd.data.gold.hasClass("active");
            if (ischeck === true) {
                return 1;
            } else {
                return 0;
        ...
        'getUseCoupon':function () {
            var ischeck = ddd.data.coupon.hasClass("active");
            if (ischeck === true) {
        ...
        //获取折扣券状态
        'getUseVoucher': function () {
            var ischeck = ddd.data.voucher.hasClass("active");
            if (ischeck === true) {
                if (chromeVersion()) {
                    var discount = $(".voucher_discount").val();
                    return parseInt($(".voucher_discount > option[value=" + discount + "]").attr('data-id'));
                } else {
                    var selectedIndex = $('.voucher_discount').get(0).selectedIndex;
                    var data = $('.voucher_discount option:selected').eq(selectedIndex).attr('data-id');
                    if (data == undefined) {
                        data = $('.voucher_discount option:selected').attr('data-id');
                    }    
                }
        ...
        'goldClick': function () {
            ...
            ddd.data.gold.on('click', function(){
                ddd.changeRadio($(this));
                ddd.changeDom();
            });
        },
        ...
        'couponClick': function () {
            ddd.data.coupon.on('click', function(){
        ...
        'voucherClick':function () {
            ddd.data.voucher.on('click', function(){
                ddd.changeRadio($(this));
                ddd.changeDom();
            });
        },
        ...
        'changeRadio':function ($this) {
            ...
            if($this.hasClass('active')){
                ...
                $(".huke-ratio .active").removeClass('active');
                $this.addClass('active');
            }
        },

        //box切换时对折扣券处理
        'handleVoucherSelect':function () {
            var voucher_discount = $(".voucher_discount");
            var selectDom = $('.crch-intro .on');
            var optShow = '';
            if (selectDom.hasClass('bod-cont-0')) {
                optShow = 'show0';
            } else if (selectDom.hasClass('bod-cont-1')) {
                optShow = 'show1';
            } else {
                optShow = 'show2';
            }
            //被禁止的优惠券价格
            var prohibitVoucher = $('.crch-intro .on').attr('data-price');
            var maxDis = 0;
            var hasChoice = false;
            var arrShow = [];
            var arrHide = [];
            $.each(voucher_discount.find('option'), function (k, v) {
                if ($(this).hasClass(optShow)) {
                    arrShow.push(k);
                ...
                if(parseInt($(this).val()) >= prohibitVoucher){
                    arrHide.push(k);
                    $(this).addClass('hide');
                }else{
            ...
            toggleOptionShow(voucher_discount, arrShow, arrHide);

            //没有可用选项
            if(!hasChoice){
                voucher_discount.val();
                voucher_discount.val(0);
                $(".huke-ratio-voucher").addClass('hide');
            }else{
                if (tagClick) {
                    voucher_discount.val(maxDis);    
                }
                
                $(".huke-ratio-voucher").removeClass('hide');
            }
        },

        //box切换时处理radio
        'changeRadioChecked':function(){

            $.each($(".huke-ratio"), function (k, v) {
                if($(this).hasClass('hide')){
                    $(this).find(".ratio-ic").removeClass('active');
                }
            });
            ...
            $.each($(".huke-ratio"), function (k, v) {
                if(!$(this).hasClass('hide')){
                    $(".huke-ratio .active").removeClass('active');
                    $(this).find(".ratio-ic").addClass('active');
                    return false;
                }
            });
        },
        ...
        //box切换处理虎课币
        'changeGoldChecked':function () {
            var gold_limit = $('.crch-intro .on').attr('data-gold-limit');
        ...
        //监听优惠券下拉值变化
        'conponDiscountClick': function () {
            $(".layout-cont").on('change',".huke-ratio .conpon_discount", function(){
                if($(this).closest('.huke-ratio').find('.ratio-ic').hasClass('active')){
                    ddd.changeDom();
                }
            });
        },
        ...
        'voucherDisCountClick': function () {
            ...
            $(".layout-cont").on('change', ".huke-ratio .voucher_discount", function () {
                ...
                if($(this).closest('.huke-ratio').find('.ratio-ic').hasClass('active')) {
                    ...
                    $(".can-use-voucher-price").html( $(".voucher_discount").val());
                }
            });
        ...
        'change': function () {
            ...
            $(".bod-cont").on('click', function () {

                var $this = $(this);
                var $container = $(".intro-bod");
                tagClick = 1;
                ...
                if ($this.hasClass('on')) {
                    ...
                    $('.crch-intro .on').removeClass('on');
                    $(this).addClass('on');

                    //处理折扣券
                    ddd.handleVoucherSelect();

                    //处理虎课币状态
                    ddd.changeGoldChecked();

                    //处理radio状态
                    ddd.changeRadioChecked();
        ...
        'changeDom': function () {
            ...
            var price = $this.attr('data-price'),
                ...
                success_url = $this.attr('data-success-url'),
                lifelong = $this.attr('data-lifelong'),
            ...
            if(is_conpon_check == true) {
                ...
                if (chromeVersion()) {
                    var vou_dis =  parseInt($(".voucher_discount").val());
                } else {
                    var selectedIndex = $('.voucher_discount').get(0).selectedIndex;
                    var vou_dis = $('.voucher_discount option:selected').eq(selectedIndex).attr('value');
                    if (vou_dis == undefined) {
                        vou_dis = $('.voucher_discount option:selected').attr('value');
                    }    
                }
                

                _price = price - parseInt(vou_dis);
                _unit_price = (_price / 365).toFixed(2);
                $(".can-use-coupon-price").html( (price * (1-  parseInt($(".conpon_discount").val() ) / 10) ).toFixed(1));
                $(".can-use-voucher-price").html(vou_dis);
            }else{
                if(Param.gold >= gold_limit){
                    if(ischeck == true){
            ...
            if(lifelong === '1'){
                var priceLifelongHtml = '付款金额：<em>'+_price + '元</em>';
                $('.crch-pay .pay-tit .price-contanier').html(priceLifelongHtml);
            }else{
                var priceHtml = '付款金额：<em>'+_price+'元/年</em>&nbsp;&nbsp;&nbsp;低至约 <em>'+_unit_price+'</em> 元/天';
                $('.crch-pay .pay-tit .price-contanier').html(priceHtml);
            ...
            //更新支付状态
            if(Param.isLogin !== '0'){
                this.data.pay.call(ddd.getData(), 'wx', success_url);
            }
        }
    };
...
//统计页面到达
$.post(Url.record_pv_uv_ip, {uv_id: Param.uv_id, model_type: Param.vip_class, uid: Param.uid},
    function () {
