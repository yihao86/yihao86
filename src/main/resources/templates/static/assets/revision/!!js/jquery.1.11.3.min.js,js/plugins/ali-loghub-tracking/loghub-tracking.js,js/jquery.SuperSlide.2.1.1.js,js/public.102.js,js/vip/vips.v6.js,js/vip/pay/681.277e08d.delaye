...
/* COMBO: filename = js/vip/vips.v6.js, type = application/javascript, resp = 200 */
// vip集合页相关js
...
$(function () {
    ...
    var vips = {
        ...
        getData :function (vip_class, tab_list) {
            return {
                type: vip_class,
                // money: $('input[data-input-js="money"]').val(),
                // useGold: this.getUseGold(),
                // useCoupon:this.getUseCoupon(),
                useVoucher: this.getUseVoucher(tab_list),
                // come_host:Param.come_host,
                come_type: Param.comeType,
                // limit_type: Param.limit_type,
                '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')};
        },
        loadQrCode: function (vip_class, container) {
            var ali = container.find(".ali");
            var wechat = container.find(".wechat");

            if(Param.isLogin !== '0'){
                //已登录
                ali.find('.code-img img').attr('src', Param.aliImage);
                wechat.find('.code-img').html('<img src="'+Param.wxImage+'" alt="">');

                this.pay(vip_class, container);
            }else{
                //未登录
                ali.find('.code-img img').attr('src', Param.aliImage);
                ...
                wechat.find('.code-img').html('<a href="javascript:void(0)" onclick="toLogin()"><img src="'+Param.wxImage+'" alt=""></a>');
            }
        },
        pay: function (vip_class, container) {

            var $this = this,
                successUrl = Url.successPrefix + '?type=' + vip_class,
                wxPayContainer = container.find('.wechat .code-img'),
                aliPayContainer = container.find('.scan-qr-code .ali');

            $this.data.pay = new payPlugin({
                payUrl : Url.vipPay,
                wxpayUrl:Url.Wxpay,
                isLogin:Param.isLogin,
                successUrl:successUrl,
                wxPayAfterUrl: Url.WxPayAfter,
                WxPayAfterMulti: Url.WxPayAfterMulti,
                csrf: $('meta[name="csrf-token"]').attr('content'),
                wxPayContainer: wxPayContainer
            });

            //绑定支付宝点击事件
            aliPayContainer.unbind('click').click(function () {
                $this.data.pay.call($this.getData(vip_class, container), 'alipay', successUrl);
            });

            $this.data.pay.call($this.getData(vip_class, container), 'wx', successUrl);
        },

        getUseVoucher: function (tab_list) {
            if (tab_list.find('.coupon').length !== 0 && !tab_list.find('.coupon').hasClass('hide')) {
                if(tab_list.find('.coupon .check-box').hasClass('active')){
                    var id = parseInt(tab_list.find('.select-main .check').attr('data-id'));
                    if (id) {
                        return id;
                    }
                }
...
// 更多特权对比弹出层
    $('.vip-privilege').on('click', '.more-btn', function() {
        $('.contrast-win').removeClass('hide');
    });
    $('.contrast-win').on('click', '.close,.win-bg', function() {
        $('.contrast-win').addClass('hide');
    });


    // vip充值类型切换
    $('.vip-box').on('click', '.head-nav', function () {

        //切换dom
        var n = $(this).index(),
            par = $(this).parent().siblings('.main').find('.tabs-list'),
            tab_id = $(this).attr('data-tab'),
            class_id = $(this).attr('data-class');

        var container = $('div[data-tab-id='+tab_id+']');

        $(this).addClass('active').siblings().removeClass('active');
        par.eq(n).removeClass('hide').siblings().addClass('hide');

        if(!$(this).hasClass('clicked')){

            if(parseInt(tab_id) === 1) {
                $(".vip-item-tabs .vip-choose .choose:eq(1)").trigger('click');
            }else if(parseInt(tab_id) === 2){
                $(".group-vip-tabs .type-vip-btn a:eq(0)").trigger('click');
            }else if(parseInt(tab_id) === 3){
                if($(".all-vip-tabs .coupon").length !== 0 && !$(".all-vip-tabs .coupon").hasClass('active')){
                    $(".all-vip-tabs .coupon .check-box").trigger('click');
                }else{
                    //更换充值二维码
                    vips.loadQrCode(
                        class_id,
            ...
            }else if(parseInt(tab_id) === 4) {
                if($(".all-svip-tabs .coupon").length !== 0 && !$(".all-svip-tabs .coupon").hasClass('active')){
                    $(".all-svip-tabs .coupon .check-box").trigger('click');
                }else{
                    //更换充值二维码
                    vips.loadQrCode(
                        class_id,
    ...
    $('.vip-choose').on('click', '.choose', function() {
        var vipid = $('.vip-item-tabs .type-vip-btn').find('.active').attr('data-vipId');
        index  = $(this).index();
        $(this).addClass('active').siblings().removeClass('active');

        var container = $(this).closest('.tabs-list'),
            class_id = $(this).attr('data-class-id');

        container.find('.init-class').val(class_id);

        var price = vipData.typeVip[vipid].price[index];
        $(".head-nav:eq(0)").attr('data-class', class_id);

        //处理无效优惠券
        if(container.find(".coupon").length !== 0){

            $.each(container.find(".coupon .select-main p"), function () {

                var show = parseInt($(this).attr('data-show')),
                    satisfy_amount = parseInt($(this).attr('data-satisfy-amount'));


                if(index === 1){
                    if(show === 1 || (show === 99999 &&  satisfy_amount <= parseInt(price))){
                ...
                if(index === 2){
                    if(show === 2 || (show === 99999 &&  satisfy_amount <= parseInt(price))){
                        $(this).removeClass('hide').addClass('show');
                    }else{
                        $(this).addClass('hide').removeClass('show');
                ...
                if(index === 0){
                    if(show === 99999 &&  satisfy_amount <= parseInt(price)){
                        $(this).removeClass('hide').addClass('show');
                    }else{
                        $(this).addClass('hide').removeClass('show');
            ...
            var classId = $(this).closest('.tabs-list').find('.init-class').val(),
                index  = $('.vip-item-tabs .choose.active').index();
            prices(classId,index);
        }
        prices(vipid,index);
        vips.loadQrCode(class_id, container);
    });
        ...
        'typeVip': [
            {
                ...
                'imgId':'1',
                'vipName':'软件入门',
                'description':'31+款软件入门教程<br>独创软件练习题<br>配套素材+源文件<br>152万人已学',
                'vipIcon':['svipIcon-software','svipIcon-software','vvipIcon-software'],
                'price':classVips[2].price,
                'classId':classVips[2].classId
            },
            {
                'id':'1',
                'imgId':'2',
                'vipName':'字体设计',
                'description':'1500+字体设计教程<br>最潮字体设计百科全书<br>配套素材+源文件<br>79万人已学',
                'vipIcon':['svipIcon-font','svipIcon-font','vvipIcon-font'],
                'price':classVips[1].price,
                'classId':classVips[1].classId
            },
            {
                'id':'2',
                'imgId':'3',
                'vipName':'海报教程',
                'description':'1000+海报设计教程<br>高逼格超实用教程集合<br>配套素材+源文件<br>54万人已学',
                'vipIcon':['svipIcon-poster','svipIcon-poster','vvipIcon-poster'],
                'price':classVips[3].price,
                'classId':classVips[3].classId
            },
            {
                'id':'3',
                'imgId':'4',
                'vipName':'综合教程',
                'description':'2500+不同类型案例教程<br>多维度案例满足不同需求<br>配套素材+源文件<br>118万人已学',
                'vipIcon':['svipIcon-synthesize','svipIcon-synthesize','vvipIcon-synthesize'],
                'price':classVips[5].price,
                'classId':classVips[5].classId
            },
            {
                'id':'4',
                'imgId':'5',
                'vipName':'C4D教程',
                'description':'1200+高逼格C4D教程<br>满足从小白到大神的进阶<br>配套素材+源文件<br>40万人已学',
                'vipIcon':['svipIcon-c4d','svipIcon-c4d','vvipIcon-c4d'],
                'price':classVips[6].price,
                'classId':classVips[6].classId
            },
            {
                'id':'5',
                'imgId':'6',
                'vipName':'图像合成',
                'description':'1000+超牛逼合成教程<br>变身为稀缺性人才<br>配套素材+源文件<br>38万人已学',
                'vipIcon':['svipIcon-image','svipIcon-image','vvipIcon-image'],
                'price':classVips[7].price,
                'classId':classVips[7].classId
            },
            {
                'id':'6',
                'imgId':'7',
                'vipName':'产品精修',
                'description':'700+超逼真精修教程<br>涵盖市场所有产品类型<br>配套素材+源文件<br>28万人已学',
                'vipIcon':['svipIcon-truing','svipIcon-truing','vvipIcon-truing'],
                'price':classVips[4].price,
                'classId':classVips[4].classId
            },
            {
                'id':'7',
                'imgId':'8',
                'vipName':'影视动画',
                'description':'800+影视级案例教程<br>覆盖栏目包装/广告/动画/特效等<br>配套素材+源文件<br>12万人已学',
                'vipIcon':['svipIcon-movies','svipIcon-movies','vvipIcon-movies'],
                'price':classVips[10].price,
                'classId':classVips[10].classId
            },
            {
                'id':'8',
                'imgId':'9',
                'vipName':'职业发展',
                'description':'300+职场职业解说<br>职场大牛的亲身传授<br>让出入职场的小白不懵逼<br>4.6万人已学',
                'vipIcon':['svipIcon-job','svipIcon-job','vvipIcon-job'],
                'price':classVips[11].price,
                'classId':classVips[11].classId
            },
            {
                'id':'9',
                'imgId':'10',
                'vipName':'人像精修',
                'description':'300+大师人像精修教程<br>电商、摄影必学的技能<br>配套素材+源文件<br>4.6万人已学',
                'vipIcon':['svipIcon-portrait','svipIcon-portrait','vvipIcon-portrait'],
                'price':classVips[14].price,
                'classId':classVips[14].classId
            },
            {
                'id':'10',
                'imgId':'11',
                'vipName':'商业插画',
                'description':'200+插画案例式教程<br>从零开始学习商业插画<br>配套素材+源文件<br>1.8万人已学',
                'vipIcon':['svipIcon-illustration','svipIcon-illustration','vvipIcon-illustration'],
                'price':classVips[15].price,
                'classId':classVips[15].classId
            },
            {
                'id':'11',
                'imgId':'12',
                'vipName':'店铺装修',
                'description':'400+最新最潮店铺教程<br>电商设计师必学专业知识<br>配套素材+源文件<br>1.9万人已学',
                'vipIcon':['svipIcon-store','svipIcon-store','vvipIcon-store'],
                'price':classVips[16].price,
                'classId':classVips[16].classId
            },
            {
                'id':'12',
                'imgId':'13',
                'vipName':'摄影艺术',
                'description':'1500+摄影前中后期教程<br>人像/风光/婚纱等你要的都有<br>配套素材+源文件<br>59万人已学',
                'vipIcon':['svipIcon-photograph','svipIcon-photograph','vvipIcon-photograph'],
                'price':classVips[9].price,
                'classId':classVips[9].classId
            },
            {
                'id':'13',
                'imgId':'14',
                'vipName':'室内设计',
                'description':'200+专业案例式教程<br>中式/日式/现代等各式风格教程<br>配套素材+源文件<br>1万人已学',
                'vipIcon':['svipIcon-indoor','svipIcon-indoor','vvipIcon-indoor'],
                'price':classVips[18].price,
                'classId':classVips[18].classId
            },
            {
                'id':'14',
                'imgId':'15',
                'vipName':'海外教程',
                'description':'120+正版授权老外教程<br>中文字幕，更便捷学习<br>配套素材+源文件<br>'+Param.foreignLearNum+'万人已学',
                'vipIcon':['svipIcon-foreign','svipIcon-foreign','vvipIcon-foreign'],
                'price':classVips[19].price,
                'classId':classVips[19].classId
            },
            {
                'id':'15',
                'imgId':'16',
                'vipName':'品牌设计',
                'description':'900+品牌设计方向教程<br>VI/logo/包装/名片等海量品牌设计的教程<br>配套素材+源文件<br>'+Param.foreignLearNum+'万人已学',
                'vipIcon':['svipIcon-brand','svipIcon-brand','vvipIcon-brand'],
                'price':classVips[20].price,
                'classId':classVips[20].classId
            },
            {
                'id':'16',
                'imgId':'17',
                'vipName':'UI设计',
                'description':'700+多维度UI设计教程<br>全面培养从小白到大神的进阶<br>配套素材+源文件<br>'+Param.foreignLearNum+'万人已学',
                'vipIcon':['svipIcon-ui','svipIcon-ui','vvipIcon-ui'],
                'price':classVips[21].price,
                'classId':classVips[21].classId
            },
        ],
        'groupVip':[
            {
                'id':'0',
                'vipName':'平面组合套餐VIP',
                'description':'(字体+海报+合成+品牌+C4D+软件 无限学习)',
                'oldPrice': '694',
                'con':'<h3>平面组合套餐立享6大分类所有教程<span>（限量20名）</span></h3><p>平面套餐=字体VIP+海报VIP+品牌VIP+C4D教程VIP+合成VIP+软件VIP（价值694元）</p><p>7000+设计教程、40+款软件入门教程以及700+独创练习题配套学习</p><p>组合购买更优惠，立享40%限时折扣</p><p>APP端可无限下载，无需网络随时随地观看，专属客服服务</p>',
                'price':classVips[801].price,
                'classId':classVips[801].classId
            },
            {
                'id':'1',
                'vipName':'电商组合套餐VIP',
                'oldPrice': '793',
                'description':'(海报+精修+人像+合成+C4D+软件+店铺 无限学习)',
                'con':'<h3>电商组合套餐立享7大分类所有教程<span>（限量20名）</span></h3><p>电商套餐=海报VIP+产品精修VIP+人像精修VIP+C4D教程VIP+合成VIP+软件VIP+店铺VIP</p><p>8000+设计教程、40款软件入门教程以及700+独创练习题配套学习</p><p>组合购买更优惠，享40%折扣</p><p>APP端视频无限下载，专属客服服务</p>',
                'price':classVips[802].price,
                'classId':classVips[802].classId
            }
        ]
    }
    // 分类vip切换
    $('.vip-item-tabs .type-vip-btn').on('click', 'a', function() {
        var vipId   = $(this).attr('data-vipId'),

            dataObj = vipData.typeVip[vipId],
            imgId   = dataObj.imgId,
            vipName = dataObj.vipName,
            des     = dataObj.description,
            classId = dataObj.classId,
            parent  = $('.vip-item-tabs .type-l');

        parent.find('.icon').attr('class', 'icon icon'+imgId);
        parent.find('h3').html(vipName+'VIP');
        parent.find('p').html(des);
        for (var i = 0; i <= 2; i++) {
            $('.choose').eq(i).find('.vip-icon').attr('class', 'vip-icon '+dataObj.vipIcon[i]);
            $('.choose').eq(i).find('.vip-name i').html(dataObj.vipName);
            $('.choose').eq(i).find('.vip-price span').html(dataObj.price[i]);
            $('.choose').eq(i).attr('data-class-id', classId[i]);
        }
        $(this).addClass('active').siblings().removeClass('active');
        $(".head-nav:eq(0)").attr('data-class', classId[1]);

        $(this).closest('.tabs-list').find('.init-class').val(classId[1]);

        $('.choose').eq(1).trigger('click');
    });
    // 组合vip切换
    $('.group-vip-tabs .type-vip-btn').on('click', 'a', function() {
        var vipId   = $(this).attr('data-vipId'),
            dataObj = vipData.groupVip[vipId],
            vipName = dataObj.vipName,
            des     = dataObj.description,
            con     = dataObj.con,
            price     = dataObj.price,
            oldPrice     = dataObj.oldPrice,
            classId     = dataObj.classId,
            container = $(this).closest('.tabs-list'),
            parent  = $('.group-vip-tabs .vip-info');
        ...
        if(container.find(".coupon").length !== 0) {
            ...
            //排除优惠券
            $.each(container.find(".coupon .select-main p"), function () {
                if (parseInt($(this).attr('data-satisfy-amount')) <= parseInt(price)) {
                    $(this).removeClass('hide').addClass('show');
                } else {
                    $(this).addClass('hide').removeClass('show');
                }
            ...
            //优惠券价格信息
            if ($(".coupon-group .check-box").hasClass('active')) {
                vourch = parseInt($(".coupon-group .select-main .check").attr('data-num'));
                $(".coupon-group .coupon-num").html('¥' + vourch);
            }
        }

        parent.find('.vip-p i').html(price);
        parent.find('.vip-p span').html('(¥ '+ oldPrice +')');

        price = price - vourch;
        pday = price/365;

        parent.find('.vip-name').html(vipName);
        parent.find('.p2').html(des);
        parent.find('.info-r').html(con);


        //订单价格显示
        container.find('.scan-qr-code .t span:eq(0)').html(price + '元/年');
        container.find('.scan-qr-code .t span:eq(1)').html(pday.toFixed(2)  + '元/天');


        $(this).addClass('active').siblings().removeClass('active');

        container = $(this).closest('.tabs-list');
        container.find('.init-class').val(classId);

        vips.loadQrCode(classId, container);
        $(".head-nav:eq(1)").attr('data-class', dataObj.classId);
    });


    function handleSelect(container){
        //处理无效优惠券
        if(container.find(".coupon .select-main .show:eq(0)").length === 0){
            container.find(".coupon").addClass('hide');
            container.find('.coupon .check-box').removeClass('active');
        }else{
            var $this = container.find(".coupon .select-main .show:eq(0)");

            var selectCon = $this.html(),
                num       = $this.attr('data-num'),
                thisContainer = $this.closest('.coupon-select');

            $this.addClass('check').siblings().removeClass('check');
            thisContainer.find('.select-head').html(selectCon);
            thisContainer.find('.select-main').hide();
            thisContainer.find('.coupon-num').html('¥' + num);

            $this.closest('.coupon').find(".check-box").addClass('active');
            container.find(".coupon").removeClass('hide');
        }
    }

// 应付金额计算
    function prices(vipid,index){
        var pdata = $('.vip-choose .active .vip-price span').html();
        var vourch = 0;
        if($(".coupon-category .check-box").hasClass('active')){
            vourch = parseInt($(".coupon-category .select-main .check").attr('data-num'));
        }
        pdata = pdata - vourch;
        qday  = pdata/365;
        ...
        if(vourch!== 0){
            $(".coupon-category .coupon-num").html('¥' + vourch);
        }
        if (index === 0) {
            $('.price-data i').html(pdata+'元');
            $('.day-p').html('');
        }else{
            $('.price-data i').html(pdata+'元/年');
            $('.day-p').html('低至<i>' + qday.toFixed(2) + '</i>元/天');
        }
    }
// 课程总数量
    (function(){
        var num = vipData.courseNum;
        $('.vip-wrapper .course-num').html(num)
    })();
// 跳转充值
    function goTo(tab){
        $('body,html').animate({scrollTop: 400}, 500);
        $('.vip-box').find('.head-nav').eq(tab).click();;
    }
// 组合vip跳转充值
    $('.group-list').on('click', '.buy-link a', function() {
        var id = $(this).attr('data-id');
        if(id === '0'){
            $(".group-vip-tabs .type-vip-btn a:eq(0)").trigger('click');
        }else{
            $(".group-vip-tabs .type-vip-btn a:eq(1)").trigger('click');
        }
        goTo(1);

    });
    $('.future-course').on('click', '.get-vip', function() {
        goTo(2);
    });
// 底部vip跳转充值
    $('.vip-value').on('click', '.open-vip', function() {
        var num = $(this).parents('.list').index();
        goTo(num);
    });
// 特权对比跳转充值
    $('.contrast-win').on('click', '.open-vip', function() {
        var num = $(this).parents('.contrast-list').index();
        $('.contrast-win').addClass('hide');
        goTo(num);
    });

    // 是否使用优惠卷
    $('.coupon').on('click', '.check-box', function() {
        ...
        if($(this).closest('.coupon').hasClass('coupon-category')){
            var index  = $('.vip-item-tabs .choose.active').index();
            prices(classId,index);
        }else{
            var vourch = 0;

            vourch = parseInt(container.find(".coupon .select-main .check").attr('data-num'));
            container.find(".coupon .coupon-num").html('¥' + vourch);

            price = parseInt(container.find('.vip-p i').html());
            //优惠券价格信息
            if (container.find(".coupon .check-box").hasClass('active')) {
                price = price - vourch;
            }

            if(container.find('.scan-qr-code .t').hasClass('lifelong')){
                container.find('.scan-qr-code .t span:eq(0)').html(price + '元');
            }else{
                container.find('.scan-qr-code .t span:eq(0)').html(price + '元/年');
                container.find('.scan-qr-code .t span:eq(1)').html((price/365).toFixed(2) + '元/天');
            }
        }


        vips.loadQrCode(container.find('.init-class').val(), container);
    });

    //点击优惠券-显示下拉
    $('.coupon-select').on('click', '.select-head', function() {
        $(this).closest('.coupon-select').find('.select-main').show();
    });

    //折扣券option选择
    $('.coupon-select').on('click', 'p', function() {
        var selectCon = $(this).html(),
            num       = $(this).attr('data-num'),
            container = $(this).closest('.tabs-list');
        ...
        container.find('.select-head').html(selectCon);
        container.find('.select-main').hide();
        container.find('.coupon-num').html('¥' + num);

        var classId = $(this).closest('.tabs-list').find('.init-class').val();

        if($(this).closest('.coupon').find(".check-box").hasClass('active')){
            vips.loadQrCode(classId, container);

            if(container.find('.coupon').hasClass('coupon-category')){
                var index  = $('.vip-item-tabs .choose.active').index();
                prices(classId,index);
            }else{
                var vourch = 0;
    ...
    var tab = parseInt(Param.tab) - 1;
    //header初始化
    $('.head-nav:eq('+tab+')').trigger('click');
});
/* COMBO: filename = js/vip/pay.multi.v3.js, type = application/javascript, resp = 200 */
/**
 * 支付插件
 * 适用多个支付的页面。合并wx-pay-after
 */
;(function($) {

    var categoryPayOrderContainer = [];
    var categoryPayInterIdentity = [];

    window.categoryPayOrderContainer = categoryPayOrderContainer;
    window.categoryPayInterIdentity = categoryPayInterIdentity;


    var payPlugin = function (opt) {
        var defaultSettings ={
            formData: {},
            payUrl: '',
            wxpayUrl:'',
            ...
            ds: 0,
            csrf:'',
            wxPayContainer: $("#wxpay")
        };

        this.options = $.extend({}, defaultSettings, opt || {});
    };

    payPlugin.prototype = {

        'call': function (formData, type, successUrl) {
            this.options.formData = formData;
            this.options.successUrl = successUrl;
        ...
        'alipay': function () {
            ...
            //状态限制
            if (1 !== $this.options.status) {
                $this.options.status = 0;
                return false;
            }

            $.ajax({
                type: 'post', url: $this.options.payUrl, data: $this.options.formData, dataType: 'json',
                success: function (response) {
        ...
        'wxpay': function () {
            ...
            $.post(
                $this.options.wxpayUrl, $this.options.formData, function (response) {

                if (0 === response.isError) {


                    $this.options.wxPayContainer.html('<img style="height:175px;width: 175px;" src="' + response.codeImgUrl + '">');

                    clearInterval(categoryPayInterIdentity);
                    $this.handleOrder(response.order_no);

                    if ($this.options.flag < 300) {
                        categoryPayInterIdentity = setInterval(function () {
                            $this.wxCallbackMulti(categoryPayOrderContainer.toString());
                            $this.options.flag = $this.options.flag + 2;
                        }, 3e3);
                    } else if ($this.options.flag >= 300 && $this.options.flag < 600) {
                        categoryPayInterIdentity = setInterval(function () {
                            $this.wxCallbackMulti((categoryPayOrderContainer.toString()));
                            $this.options.flag = $this.options.flag + 2;
                        }, 1e4);
        ...
        'handleOrder': function (order_no) {

            var len = parseInt(categoryPayOrderContainer.length);
            if(len === 10){
                categoryPayOrderContainer.splice(0, 1);
            }
            categoryPayOrderContainer.push(order_no);
        },

        //微信回掉
        'wxCallback': function (order_no) {

            var $this = this;

            if (this.options.flag >= 600) {
                clearInterval(this.options.ds);
            }

            $.ajax({
                type: "post",
                url: $this.options.wxPayAfterUrl,
                data: {
                    order_no: order_no,
                    '_csrf-frontend': $this.options.csrf
                },
                dataType: "json",
                success: function (response) {
                    if (response.isError === 0) {
        ...
        'wxCallbackMulti': function (orders) {
            ...
            $.ajax({
                type: "post",
                url: $this.options.WxPayAfterMulti,
                data: {
                    order_no: orders,
                    '_csrf-frontend': $this.options.csrf
                },
                ...
                success: function (response) {
                    if (response.isError === 0) {
                        clearInterval(window.categoryPayInterIdentity);
                        window.location.href = response.successUrl;
                    }
                }
            });
    ...
    window.payPlugin = payPlugin;

    $.fn.pay = function(options)  {
        var pay = new payPlugin(options);
        return pay.call();
    };
})(jQuery);
