...
/* COMBO: filename = static/practice/index.static, type = application/javascript, resp = 200 */
/*
*   每周一练
**/
$(function () {
    ...
    **/
    ...
    //点赞
    $('#static-detail-priase').one('click', function () {
        ...
        $.post(Url.detailUrl, {
            work_id: _self.attr('data-id'),
            "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")
        }, function (b) {
            console.log(b);
            if (0 == b.isError) {

                _self.removeClass('yzan-style');
                $("#static-detail-priase #number").html(b.number);
            }
    ...
    //收藏
    $('.share-way .collect-btn').on('click', function () {
        ...
        var _self = $(this);
        $.post(Url.collectionUrl, {
            work_id: _self.attr('data-id'),
            "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")
        }, function (b) {
            if (0 == b.isError) {
                if (b.oper) {
                    _self.removeClass('collected');
            ...
            $(".share-way .collect-btn #collectNmuber").html(b.number);
        }, "json");
    });
    ...
    //     $(this).parent().before(
    //         '<div class=' + "uploading" + '>' +
    //         '<img src=' + "images/timg.jpg" + ' alt= ' + "已上传图片" + '>' +
    //         '<div class=' + "del-pop" + '><i class=' + "icont-bigdel" + '></i> 删除</div>' +
    //         '</div>'
    //     )
    //     if ($('.uploading').length >= 3) {
    //         $(this).parent().hide();
    //     };
    //     $('.imgs-box .uploading').click(function() {
    //         $(this).remove();
    ...
    // input判断字数
    $("#tit_num").keyup(function () {

        var length = this.value.length;
        $('.upld-tit .words-num').text(length + '/15');

    });
    // txt
    $("#con_num").keyup(function () {

        var length = this.value.length;
        $('.upld-con .words-num').text(length + '/200');
    ...
    //关注
    $('#cared-none').on('click', function () {
        if (0 == Param.is_login) {
            $("#loginModal").removeClass("hide");
            return false;
        }
        var _this = $(this);
        var span = $(".gz-num .gz-num-count");
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 1) {
                    _this.addClass('hide');
                    $('#cared-delet').removeClass('hide');
//                    console.log(res.number);
                    span.text(res.number);
                    return false;
                }
            }
        });
...
//                    console.log(res.number);
    ...
    $('#release_btn').click(function () {
        var titNum = $('#tit_num').length,
            upNum = $('.uploading').length;
        if (titNum == 0 || upNum == 0) {
            $(document).scrollTop(0);

            if (titNum == 0) {
                $('.upld-tit .alert-con').show();
            }
            if (upNum == 0) {
                $('.upld-imgs .alert-con').show();
            }
        }
    ...
    // var $cutSuggest = $('.static-suggest'),
    //     $cutText = $cutSuggest.text(),
    //     $winWords = $('.win-list .win-words'),
    //     $winText = $winWords.text();
    //
    // cutString(34, $cutText, $cutSuggest);
    // cutStringWinWords(109, $winText, $winWords);
    //
    // function cutString(len, str, nStr) {
    //     if (str.length > len) {
    ...
    var $zanBtn = $('.teacher-info .add-gz-btn'),
        $closeBtn = $('.home-con .close-con'),
        $zanSpan = $zanBtn.find('span'),
        $icont = $zanBtn.find('i'),
        $cloSpan = $closeBtn.find('span');

    $closeBtn.click(function () {
        $(this).parent().find('.close-list').toggleClass('open-sty');
        $(this).find('i').toggleClass('close-con-sty');
        if ($(this).parent().find('.close-list').hasClass('open-sty')) {
            $cloSpan.html('收起详情');
    ...
    var $winBtn = $('#win-box'),
        $allBtn = $('#all-box'),
        $allBox = $('.all-zp .all-box'),
        $winBox = $('.all-zp .win-box'),
        $banBox = $('.all-zp .px-arguments'),
        $allPagesBox = $('.all-pages'),
        $prizePagesBox = $('.prize-pages'),
        $thSly = 'allzp-this';


    function changePag(pagBtn, thStl, pagThis, pagthat, thatStl, ban, all, prize) {

        pagBtn.click(function () {
            $(this).addClass(thStl);
            pagThis.show();
            pagthat.hide();
            thatStl.removeClass(thStl);
            if (ban.hasClass('hide')) {
    ...
    changePag($winBtn, $thSly, $winBox, $allBox, $allBtn, $banBox, $allPagesBox, $prizePagesBox);
    changePag($allBtn, $thSly, $allBox, $winBox, $winBtn, $banBox, $allPagesBox, $prizePagesBox);
     ...
     * 首页点击作品列表页
     */
    $(".practice-home-go-detail").on('click', function () {
        var $id = $(this).attr('data-id');
        if (Param.is_login == 0) {
            $('#loginModal').removeClass('hide');
            return false;
        }
        window.open('/practice/detail/' + $id + '.html');
    });
     ...
     * 删除作品的确认框
     */
    $("#practice-delete-work").on('click', function () {
        var $model = $("#practice-delete-work-model");
        var id = $(this).attr('data-id');
        if (Param.is_login == 0) {
            $('#loginModal').removeClass('hide');
            return false;
        ...
        $model.find("#static-delete-id").val(id);
    });

    /**
     * 删除作品js
     */
    $("#practice-delete-work-model #delete-work-confirm").on('click', function () {
        $.ajax({
            url: Url.WorkDeleteUrl,
            data: {
                "_csrf-frontend": $('meta[name="csrf-token"]').attr("content"),
                work_id: $("#practice-delete-work-model #static-delete-id").val()
            },
            type: 'POST',
            dataType: 'JSON',
            success: function (res) {
                ...
                if (res.isError == 1) {
                    $("#practice-delete-work-model").addClass('hide');
                    $("#practice-delete-work-model-fail").removeClass('hide');
                    $("#practice-delete-work-model-fail .ts-cont").html(res.message);
                } else {
    ...
    $('.practice-handle-stick').on('click', function () {
        var _self = $(this);
        $.post(Url.StickMark, {
            "id": _self.closest('.content').attr('data-id'),
            "type": 'up',
            "_csrf-frontend": $('meta[name="csrf-token"]').attr("content")
    ...
    $('.practice-cancel-stick').on('click', function () {
        var _self = $(this);
        $.post(Url.StickMark, {
            "id": _self.closest('.content').attr('data-id'),
            "type": 'down',
