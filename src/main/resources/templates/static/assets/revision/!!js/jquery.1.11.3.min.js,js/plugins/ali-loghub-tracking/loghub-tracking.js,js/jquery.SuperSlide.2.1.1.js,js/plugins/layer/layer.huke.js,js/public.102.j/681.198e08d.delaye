...
/* COMBO: filename = js/plugins/layer/layer.huke.js, type = application/javascript, resp = 200 */
...
/* COMBO: filename = js/plugins/jquery/jquery.scroll.floor.v2.js, type = application/javascript, resp = 200 */
(function(window){
	var defaults = {
		floorClass : '.scroll-floor',
		navClass : '.scroll-nav a',
		activeClass : 'active',
		activeTop : 300,
		scrollTop : -100,
		delayTime : 500
...
/* COMBO: filename = js/task/task.v5.js, type = application/javascript, resp = 200 */
$('.lazy').lazyload({threshold: 200});
// 关闭限制弹窗
$('.task-win').on('click', '.close', function () {
    $('.task-win').addClass('hide');
});

// 关闭作业评改提示弹窗
$('.task-tips').on('click', '.close', function() {
    $(this).parents('.task-tips').addClass('hide');
});
...
function taskFixed() {
    var scrollTop = $(document).scrollTop();
    if (scrollTop > 140) {
        $(".task-fixed").removeClass("hide");
    }
    else {
        $(".task-fixed").addClass("hide");
    }
}
...
//首页评改提示框-上传作品
$('.help-win').on('click', '.alink', function() {

    // $(".task-win").removeClass('hide');
    if(Param.uid == 0){
        toLogin();
        return false;
    }
    ...
    if(Param.isAll === '0' && Param.hasFree === '0'){
        $('.help-win').addClass('hide');
        $(".task-win[data-type=1]").removeClass('hide');
        return false;
    }
    console.log(222);
    if(Param.hasUpload === '1'){
        $('.help-win').addClass('hide');
        $(".task-win[data-type=2]").removeClass('hide');
        return false;
    }
...
//首页评改提示框-关闭按钮
$('.help-win').on('click', '.close,.win-bg', function() {
    $('.help-win').addClass('hide');
});
...
function getPath(obj, fileQuery, transImg) {
    ...
    reader.onload = function (e) {
        ...
        if (strSrc.localeCompare('jpg') === 0 || strSrc.localeCompare('jpeg') === 0 || strSrc.localeCompare('gif') === 0 || strSrc.localeCompare('png') === 0) {
            ...
            $('.form-bot .error-tips').html('文件类型错误，请上传jpg，jpeg，gif，png类型图片');
        }
    }
...
function show() {
    ...
    $('.form-bot .error-tips').html('替换成功，请点击提交按钮上传');
}



$(".upload-task").on('click', function () {
    ...
    if(Param.isAll === '0' && Param.hasFree === '0'){
        sendStatisticsCommonFunction(Url.statisticsPopup, {type: 1,pageType:1,'uv_id':Param.uv_id}, true);
        $(".task-win[data-type=1]").removeClass('hide');
        return false;
    }
    ...
    window.open(Url.homeSourceUpload);
});


$(".upload-fix-task").on('click', function () {
    ...
    window.open(Url.fixSourceUpload);
});

$('.follw-btn .unfollow').click(function () {
    if(Param.uid == 0){
        toLogin();
        return false;
    }
    ...
    $.ajax({
        url: Url.followUrl,
        data: {"_csrf-frontend": $('meta[name=csrf-token]').attr('content'), "UserFlowerTeacherForm[tid]": teacherId},
        type: 'POST',
        dataType: 'JSON',
        success: function (res) {
            if (res.status == 1) {
...
$('.follw-btn .refollw').click(function () {
    if(Param.uid == 0){
        toLogin();
        return false;
    }
...
/**
 * 作业评改-首页-ajax图文列表
 * @type {{init: ListLoad.init, ajaxGetData: ListLoad.ajaxGetData}}
 */
var ListLoad = {
    init:function (page) {
        var $this = this;
        ...
        //排序按钮
        $("#taskSort a").on('click', function () {
            if($(this).hasClass('active')){
                return false;
            }
            $("#taskSort .active").removeClass('active');
            $(this).addClass('active');
            $this.ajaxGetData(0);
        });
    },
    ...
    ajaxGetData: function (page) {
        ...
        var sort = $("#taskSort .active").attr('data-sort-id');

        $.ajax({
            url: Url.ajaxGetList,
            data: {sort:sort,pages:page,webp:Param.webp, "_csrf-frontend": $('meta[name=csrf-token]').attr('content')},
            type: 'post',
            dataType: 'JSON',
            success: function (response) {
                ...
                $.each(response.list, function (k, v) {
                    html += '' +
                        ...
                        '    </div>' +
                        '    <div class="study-num">' +
                        '         <span class="fl">'+v.tag+'</span><span class="fr">'+v.study_num+'人已学习</span>' +
                        '    </div>' +
                        '</div>';
                });

                $("#imgshow-sec-container").html(html);
                $("#page-m").html(response.pageHtml);

                //分页按钮
                $('a.page_click_class').on('click', function () {
                    var page = $(this).attr('data-page');
                    $this.ajaxGetData(page);
                    $('html, body').animate({
                        scrollTop: $('.section-scroll-contanier').offset().top - 140
                    }, 400);
                });
            }
        });
